...
def get_datasets(get_train_images_and_masks, get_test_images_and_masks, batch_size, center_crop_size, augmentation = False):
    ...
    train_dataset = GeneralDataset(training_images, training_masks, center_crop_size, augmentation)
    train_dataloader = DataLoader(train_dataset, batch_size = batch_size, pin_memory = True, shuffle = True)
    
    if test_images is not None:
        test_dataset = GeneralDataset(test_images, test_masks, center_crop_size, augmentation = False)
        test_dataloader = DataLoader(test_dataset, batch_size = batch_size, pin_memory = True, shuffle = True)
