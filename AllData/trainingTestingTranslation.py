...
def get_training_images_and_masks():
    ...
    images = [(os.path.join(training_path, image_dir, image), os.path.join(training_path, image_dir.replace("t1n", "t1n'"), image)) for image_dir in os.listdir(training_path) for image in os.listdir(os.path.join(training_path, image_dir)) if image_dir.endswith('t1n')]
...
def get_testing_images_and_masks():
    ...
    images = [(os.path.join(training_path, image_dir.replace("t2f", "t2f'"), image), os.path.join(training_path, image_dir, image)) for image_dir in os.listdir(training_path) for image in os.listdir(os.path.join(training_path, image_dir)) if image_dir.endswith('t2f')]
    
    images = [image for image in images if not is_blank(image[1].replace('-t2f', '-seg'))]
    
    masks = [image[1].replace('-t2f', '-seg') for image in images]
...
model = {"model": UNet2d(in_channels = 2, n_classes = 1, n_channels = 16).to('cuda'), "name":"trainingTestingTranslation"}
