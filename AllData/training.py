...
class Trainer:
    ...
    def _compute_loss_and_outputs_with_selection(self, images: torch.Tensor, targets: torch.Tensor, selections: torch.Tensor):
        ...
        logits = self.net(images)
        confidence_scores = 4 * (torch.abs(selections - 0.5)**2)
        loss = nn.BCEWithLogitsLoss(weight = confidence_scores)(logits, targets)
        return loss
    
    def get_confidence(self, prob):
        confidence_scores = 4 * (torch.abs(prob - 0.5)**2)
